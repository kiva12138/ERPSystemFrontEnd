  ...
  <span v-loading="dataLoading">
    <div  style="vertical-align: center; margin-top: 1%; text-align: center;">
      ...
      <div style="width: 30%; text-align: center; display: inline-block;">
        ...
        <div>
          <span style="font-size: 100%; color: #303133;">正在准备生产</span>
        </div>
      </div>
      ...
      <el-collapse accordion>
        <el-collapse-item v-for="(item, index) in billData.waitingBills" v-bind:key="index">
          <template slot="title">
            ...
            <i class="header-icon el-icon-info" style="padding-left: 20px; color: #E6A23C;"></i>
            <span style="padding-left: 10px; color: #E6A23C;">已创建，正在准备生产</span>
          </template>
...
export default {
  ...
  methods: {
    reloadData () {
      ...
      this.$axios({
        method: 'get',
        url: this.GLOBAL.backEndIp + '/api/produce/getwaitingbill'
      }).then(response => {
